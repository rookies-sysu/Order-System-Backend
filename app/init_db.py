...
def init_db():
    ...
    for rInfo in rDB:
        ...
        rOpt.insertRestaurantItem(restaurantName=rInfo["restaurantName"],
                                  ...
                                  email=rInfo["email"])
        tOpt = tableOperator(
            restaurantName=rInfo["restaurantName"], password=rInfo["password"])
    ...
    for mInfo in mDB:
        rOpt = restaurantOperator()
        rOpt.manageRestaurantTable(
            restaurantName="TINYHIPPO", password="123456")
        ...
        _, result = selectOperator(
            tableName="DishType", dishTypeName=mInfo["name"], restaurantID=restaurantID, result=["dishTypeID"])
    ...
    for rcInfo in rcDB["data"]:
        ...
        for obj in rcInfo['details']:
            rcdOpt.insertRecommendationDetailsItem(
                recommendationID=rcid, dishID=obj["dish_id"], description=obj["description"])
